...
yearList = {'2024'}; 
saveDir = 'D:\JCW\Projects\KEPCO_ESS_Local\Dataset_Overview\ChgDchg_Search_NewData';

C_nom_cell = 64;
Np = 2;
Ns = 17*14;
soc_threshold = 5;  % SOC variation threshold (%)
...
for year_idx = 1:length(yearList)
    ...
    for i = 1:length(month_folders)
        ...
        if length(month_name) >= 6
            ...
            % Process all months for NewData
    ...
    for month_idx = 1:length(valid_months)
        ...
        for file_idx = 1:length(mat_files)
            ...
            try
                ...
                current_data = event_data.DCCurrent / Np;  % Convert to cell level
                time_data = event_data.Date_Time;  % duration type
                voltage_data = event_data.CVavg;
                power_data = event_data.DCPower / Ns;
                temp_data = event_data.MTavg;
                ...
                valid_time = time_data(valid_mask);  % duration type
