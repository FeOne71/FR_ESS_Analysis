...
min_charge_sec = 600;               % 연속 1064*분 이상 충전 구간
dt = 1;                             % s (고정 가정)
...
dataFile_0607 = 'D:\JCW\Projects\KEPCO_ESS_Local\Rack_raw2mat\Old\2021\202106\Raw_20210607.mat';
...
dates = {'0603', '0607'};
dataFiles = {dataFile_0603, dataFile_0607};
base_dates = {datetime(2021,6,3), datetime(2021,6,7)};
...
fprintf('Found %d charge segments before filtering (min duration: %d sec)\n', size(chgSegs,1), min_charge_sec);
chgSegs = chgSegs(dur(chgSegs) >= min_samples, :);
fprintf('Found %d charge segments after filtering (>= %d sec)\n', size(chgSegs,1), min_charge_sec);
if size(chgSegs,1) > 0
    for k = 1:size(chgSegs,1)
        fprintf('  Charge segment %d: idx %d-%d (duration: %.1f min, time: %s to %s)\n', ...
            k, chgSegs(k,1), chgSegs(k,2), (chgSegs(k,2)-chgSegs(k,1)+1)/60, ...
            datestr(t(chgSegs(k,1)), 'HH:MM:SS'), datestr(t(chgSegs(k,2)), 'HH:MM:SS'));
    end
end
...
%% For each charge segment, compute anchors and Qmax using original current and filtered voltage
Results = [];
rows = {};
for k = 1:size(chgSegs,1)
    ...
    fprintf('  Debug (Filtered): befChg=%d (time=%s), continuous_idle_start=%d (time=%s), rest_duration=%.1f min\n', ...
        befChg, datestr(t(befChg), 'HH:MM:SS'), continuous_idle_start, datestr(t(continuous_idle_start), 'HH:MM:SS'), ...
    ...
    % === 디버깅: 실제 연속 휴지구간 정보 ===
    fprintf('\n=== Debugging Segment %d (Date: %s, Original Current) ===\n', k, date_str);
    ...
    % Second rest period (after charge) - 전체 휴지구간 길이
    ...
    Results(idx).chg_start = chg_start;
    Results(idx).chg_end   = chg_end;
    Results(idx).befChg = befChg;  % 충전 전 휴지구간 마지막 시점
    Results(idx).aftChg = aftChg;  % 충전 후 휴지구간 마지막 시점
    Results(idx).V1 = Vcell_avg(befChg); Results(idx).V2 = Vcell_avg(aftChg);  % 원본 전압 (Qmax 계산용)
    Results(idx).I1 = I_cell(befChg); Results(idx).I2 = I_cell(aftChg);  % 원본 전류
    ...
    Results(idx).rest1_duration = rest1_duration; Results(idx).rest2_duration = rest2_duration;
...
%% Visualization
fig = figure('Name',sprintf('FieldQmax Rack01 (2021-%s)', date_str),'NumberTitle','off');
...
if numSeg == 0
    ...
    for s = 1:numSeg
        ...
        tblData{16,s} = sprintf('%.4f', Results(s).I1);  % 원본 전류
        tblData{17,s} = sprintf('%.4f', Results(s).I2);  % 원본 전류
...
% Save figure
saveas(fig, sprintf('Qmax/charge/FieldQmax_Figure_2021_%s.fig', date_str));
...
save(sprintf('Qmax/charge/FieldQmax_Results_2021_%s.mat', date_str), 'Results', 'T', 'tblData', 'varNames', 'colNames');

%% Figure: 원본 vs 필터링된 전압 비교
fig_voltage_comp = figure('Name', sprintf('Voltage Comparison: Raw vs Filtered (2021-%s)', date_str), 'NumberTitle', 'off');
...
saveas(fig_voltage_comp, sprintf('Qmax/charge/FieldQmax_Voltage_Comparison_2021_%s.fig', date_str));
...
for seg_idx = 1:numel(Results)
    fig_rest_voltage = figure('Name', sprintf('Rest Period Voltage Analysis: Segment %d (2021-%s)', seg_idx, date_str), 'NumberTitle', 'off');
    ...
    % 휴지기 2 인덱스 계산
    indices_rest2 = chg_end:rest2_end;
    ...
    if isfield(Results(seg_idx), 'aftChg')
        aftChg = Results(seg_idx).aftChg;
    ...
    saveas(fig_rest_voltage, sprintf('Qmax/charge/FieldQmax_Rest_Voltage_Segment%d_2021_%s.fig', seg_idx, date_str));
end

%% SOC2 안정화 비교 분석 (긴 휴지기와 비교)
fprintf('\n=== SOC2 Stabilization Analysis (2021-%s) ===\n', date_str);
...
fig2 = figure('Name', sprintf('SOC2 Stabilization Comparison (2021-%s)', date_str), 'NumberTitle', 'off');
...
saveas(fig2, sprintf('Qmax/charge/FieldQmax_SOC2_Comparison_2021_%s.fig', date_str));

% 업데이트된 Results 저장
save(sprintf('Qmax/charge/FieldQmax_Results_Extended_2021_%s.mat', date_str), 'Results', 'T', 'tblData', 'varNames', 'colNames');

%% Figure 3: 필터링된 전압, 원본 전류 시각화
fig3 = figure('Name', sprintf('Filtered Voltage & Raw Current Overview (2021-%s)', date_str), 'NumberTitle', 'off');

% 원본 전류로 충전/유휴 구간 재계산 (원본 로직과 동일)
% 원본 전류로 마스크 및 구간 재계산
isIdle_filtered = abs(I_cell) < thr_cell;
isChg_filtered = I_cell > thr_cell;
idleSegs_filtered = find_segments(isIdle_filtered);
chgSegs_filtered = find_segments(isChg_filtered);

% Filter charge segments by duration >= 600 s (dt=1s)
dur = @(seg) seg(:,2) - seg(:,1) + 1;   % samples
min_samples = ceil(min_charge_sec / dt);
fprintf('Found %d filtered charge segments before filtering (min duration: %d sec)\n', size(chgSegs_filtered,1), min_charge_sec);
chgSegs_filtered = chgSegs_filtered(dur(chgSegs_filtered) >= min_samples, :);
fprintf('Found %d filtered charge segments after filtering (>= %d sec)\n', size(chgSegs_filtered,1), min_charge_sec);
if size(chgSegs_filtered,1) > 0
    for k = 1:size(chgSegs_filtered,1)
        fprintf('  Filtered charge segment %d: idx %d-%d (duration: %.1f min, time: %s to %s)\n', ...
            k, chgSegs_filtered(k,1), chgSegs_filtered(k,2), (chgSegs_filtered(k,2)-chgSegs_filtered(k,1)+1)/60, ...
            datestr(t(chgSegs_filtered(k,1)), 'HH:MM:SS'), datestr(t(chgSegs_filtered(k,2)), 'HH:MM:SS'));
    end
end
...
for k = 1:size(chgSegs_filtered, 1)
    ...
    fprintf('  Debug (Filtered): befChg_filtered=%d (time=%s), continuous_idle_start_filtered=%d (time=%s), rest_duration=%.1f min\n', ...
        befChg_filtered, datestr(t(befChg_filtered), 'HH:MM:SS'), continuous_idle_start_filtered, datestr(t(continuous_idle_start_filtered), 'HH:MM:SS'), ...
        actual_rest_duration_sec/60);
    if actual_rest_duration_sec < 180  % 3분 미만이면 스킵
        fprintf('Skipping segment %d: rest duration too short (%.1f min)\n', k, actual_rest_duration_sec/60);
        continue;
...
if ~isempty(Results_filtered) && numel(Results_filtered) > 0
    min_time = t(min([Results_filtered.befChg])) - hours(1);
    max_time = t(max([Results_filtered.aftChg])) + hours(1);
    valid_time_indices = (t_mid >= min_time) & (t_mid <= max_time);
...
saveas(fig3, sprintf('Qmax/charge/FieldQmax_Filtered_Overview_2021_%s.fig', date_str));

%% Figure 4: 충전 시작 전/후 dV/dt 분석 (필터링된 데이터 기준)
fig4 = figure('Name', sprintf('Pre/Post Charge dV/dt Analysis (2021-%s)', date_str), 'NumberTitle', 'off');
...
saveas(fig4, sprintf('Qmax/charge/FieldQmax_SOC2_Analysis_2021_%s.fig', date_str));

% 최종 Results 저장
save(sprintf('Qmax/charge/FieldQmax_Results_Final_2021_%s.mat', date_str), 'Results', 'T', 'tblData', 'varNames', 'colNames');
