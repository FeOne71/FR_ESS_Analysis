%% FieldQmax_Newdata_discharge.m - Qmax estimation from field data (New data format, Discharge)
% Estimates Qmax using SOC change between rest periods (befDischg to aftDischg)
...
OCV_data = T.OCV_data;
    ...
    % Find discharge segments (I_rack < -thr_A for min_discharge_sec)
    discharge_mask = I_rack < -thr_A;
    dischgSegs = local_find_segments(discharge_mask);

    % Filter by minimum duration
    valid_segs = [];
    for k = 1:size(dischgSegs,1)
        seg_duration = dischgSegs(k,2) - dischgSegs(k,1) + 1;
        if seg_duration >= min_discharge_sec
            valid_segs(end+1) = k;
        end
    end
    dischgSegs = dischgSegs(valid_segs,:);
    ...
    for k = 1:size(dischgSegs,1)
        ...
        prevIdleIdx = find(idleSegs(:,2) < dischg_start, 1, 'last');  % 방전 시작 이전 휴지 구간
        nextIdleIdx = find(idleSegs(:,1) > dischg_end, 1, 'first');    % 방전 종료 이후 휴지 구간
        
        if isempty(prevIdleIdx) || isempty(nextIdleIdx)
            % Skip if either anchor missing
            if isempty(prevIdleIdx)
                fprintf('Skipping segment %d: no idle segment before discharge\n', k);
            end
            if isempty(nextIdleIdx)
                fprintf('Skipping segment %d: no idle segment after discharge\n', k);
            end
            continue;
        end

        % 방전 시작 직전 휴지 구간의 마지막 시점 사용
        befDischg = idleSegs(prevIdleIdx, 2);  % 방전 전 휴지구간 마지막 시점
        continuous_idle_start = idleSegs(prevIdleIdx, 1);  % 방전 전 휴지구간 시작 시점
        ...
        V_aftDischg = Vcell_avg(aftDischg);  % 방전 후 휴지 종료 시점 전압
        I_befDischg = I_cell(befDischg);     % 방전 전 휴지 종료 시점 전류
        I_aftDischg = I_cell(aftDischg);     % 방전 후 휴지 종료 시점 전류

        % SOC at idle end points (OCV-based and BMS)
        SOC1 = SOC_from_OCV(V_befDischg);   % 방전 전 휴지 종료 시점 SOC (OCV-based)
        SOC2 = SOC_from_OCV(V_aftDischg);   % 방전 후 휴지 종료 시점 SOC (OCV-based)
        SOC1_raw = SOC_BMS(befDischg);      % Raw SOC at befDischg
        SOC2_raw = SOC_BMS(aftDischg);      % Raw SOC at aftDischg
        ...
        % Second rest period (after discharge)
        ...
        Results(k).dischg_start = dischg_start;
        Results(k).dischg_end   = dischg_end;
        Results(k).befDischg = befDischg;  % 방전 전 휴지구간 마지막 시점
        Results(k).aftDischg = aftDischg;  % 방전 후 휴지구간 마지막 시점
        Results(k).V1 = Vcell_avg(befDischg); Results(k).V2 = Vcell_avg(aftDischg);
        Results(k).I1 = I_cell(befDischg); Results(k).I2 = I_cell(aftDischg);
        ...
        rows(end+1, :) = {k, datestr(t(dischg_start)), datestr(t(dischg_end)), ...
            char(rest1_duration), char(rest2_duration), SOC1, SOC2, dSOC_row, SOC1_raw, SOC2_raw, dSOC_row_BMS, Qmax_cell_Ah, Qmax_cell_Ah_BMS, Q_Ah_cell, Vcell_avg(dischg_start), Vcell_avg(dischg_end), I_cell(dischg_start), I_cell(dischg_end), SOH_end, SOH_end}; %#ok<AGROW>
    end

    %% Visualization
    fig = figure('Name',sprintf('FieldQmax Discharge Rack01 (%s)', year),'NumberTitle','off');
    ...
    % Save figure
    saveas(fig, sprintf('Qmax/discharge/FieldQmax_Discharge_Figure_%s.fig', year));
    ...
    save(sprintf('Qmax/discharge/FieldQmax_Discharge_Results_%s.mat', year), 'Results', 'T', 'tblData', 'varNames', 'colNames');
    ...
    fig_voltage_comp = figure('Name', sprintf('Voltage Comparison: Raw vs Filtered Discharge (%s)', year), 'NumberTitle', 'off');
    ...
    saveas(fig_voltage_comp, sprintf('Qmax/discharge/FieldQmax_Discharge_Voltage_Comparison_%s.fig', year));
    ...
    for seg_idx = 1:numel(Results)
        fig_rest_voltage = figure('Name', sprintf('Rest Period Voltage Analysis Discharge: Segment %d (%s)', seg_idx, year), 'NumberTitle', 'off');
        ...
        aftDischg = Results(seg_idx).aftDischg;  % SOC2 시점
        ...
        saveas(fig_rest_voltage, sprintf('Qmax/discharge/FieldQmax_Discharge_Rest_Voltage_Segment%d_%s.fig', seg_idx, year));
    end
    
    %% SOC2 안정화 비교 분석 (긴 휴지기와 비교)
    fprintf('\n=== SOC2 Stabilization Analysis (%s, Discharge) ===\n', year);
    ...
    fig2 = figure('Name', sprintf('SOC2 Stabilization Comparison Discharge (%s)', year), 'NumberTitle', 'off');
    ...
    saveas(fig2, sprintf('Qmax/discharge/FieldQmax_Discharge_SOC2_Comparison_%s.fig', year));
    
    %% Figure 3: 필터링된 전압, 원본 전류 시각화
    fig3 = figure('Name', sprintf('Filtered Voltage & Raw Current Overview Discharge (%s)', year), 'NumberTitle', 'off');
    
    % 원본 전류로 방전/유휴 구간 재계산 (원본 로직과 동일)
    % 원본 전류로 마스크 및 구간 재계산
    isIdle_filtered = abs(I_cell) < thr_A / Np;
    isDischg_filtered = I_cell < -thr_A / Np;
    idleSegs_filtered = local_find_segments(isIdle_filtered);
    dischgSegs_filtered = local_find_segments(isDischg_filtered);

    % Filter discharge segments by minimum duration
    valid_segs = [];
    for k = 1:size(dischgSegs_filtered,1)
        seg_duration = dischgSegs_filtered(k,2) - dischgSegs_filtered(k,1) + 1;
        if seg_duration >= min_discharge_sec
            valid_segs(end+1) = k;
        end
    end
    dischgSegs_filtered = dischgSegs_filtered(valid_segs,:);
    ...
    for k = 1:size(dischgSegs_filtered, 1)
        ...
        prevIdleIdx = find(idleSegs_filtered(:,2) < dischg_start, 1, 'last');  % 방전 시작 이전 휴지 구간
        nextIdleIdx = find(idleSegs_filtered(:,1) > dischg_end, 1, 'first');    % 방전 종료 이후 휴지 구간

        if isempty(prevIdleIdx) || isempty(nextIdleIdx)
            % Skip if either anchor missing
            if isempty(prevIdleIdx)
                fprintf('Skipping segment %d (Filtered): no idle segment before discharge\n', k);
            end
            if isempty(nextIdleIdx)
                fprintf('Skipping segment %d (Filtered): no idle segment after discharge\n', k);
            end
            continue;
        end

        % 방전 시작 직전 휴지 구간의 마지막 시점 사용 (원본 전류 기준)
        befDischg_filtered = idleSegs_filtered(prevIdleIdx, 2);  % 방전 전 휴지구간 마지막 시점
        continuous_idle_start_filtered = idleSegs_filtered(prevIdleIdx, 1);  % 방전 전 휴지구간 시작 시점
    ...
    saveas(fig3, sprintf('Qmax/discharge/FieldQmax_Discharge_Filtered_Overview_%s.fig', year));
    
    % 업데이트된 Results 저장
    save(sprintf('Qmax/discharge/FieldQmax_Discharge_Results_Extended_%s.mat', year), 'Results', 'T', 'tblData', 'varNames', 'colNames');

    %% Figure 4: 방전 시작 전/후 dV/dt 분석 (필터링된 데이터 기준)
    fig4 = figure('Name', sprintf('Pre/Post Discharge dV/dt Analysis (%s)', year), 'NumberTitle', 'off');
    ...
    saveas(fig4, sprintf('Qmax/discharge/FieldQmax_Discharge_SOC2_Analysis_%s.fig', year));
    
    % 최종 Results 저장
    save(sprintf('Qmax/discharge/FieldQmax_Discharge_Results_Final_%s.mat', year), 'Results', 'T', 'tblData', 'varNames', 'colNames');
