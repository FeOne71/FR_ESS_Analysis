...
% --- 이벤트 검출 파라미터 ---
idle_thr_cell = 64*0.02;  % 휴지 상태 전류 임계값 (1.92A)
min_duration_sec = 30;  % 이벤트로 인정할 최소 지속 시간 (초)
min_idle_duration_sec = 60;  % 이벤트 시작 전 최소 휴지 구간 (초)

% --- 데이터 필터링 방법 선택 ---
% 'sg' : Savitzky-Golay 필터 (기본값, 엣지 유지에 유리)
% 'movmean' : 이동평균 필터 (단순 평활화)
smoothing_method = 'movmean';  % 'sg' or 'movmean'

% --- 필터링 파라미터 ---
% SG 필터 파라미터 (smoothing_method = 'sg'일 때 사용)
sg_window = 5;      % SG 필터 윈도우 크기 (홀수, 권장: 5)
sg_order = 1;       % SG 필터 다항식 차수 (권장: 1)

% 이동평균 필터 파라미터 (smoothing_method = 'movmean'일 때 사용)
movmean_window = 5; % 이동평균 윈도우 크기 (초 단위, 권장: 5)

% =========================================================================

%% 2. 필터링 방법 확인 및 출력
fprintf('=== Event Detection Configuration ===\n');
fprintf('Smoothing method: %s\n', upper(smoothing_method));
if strcmpi(smoothing_method, 'sg')
    fprintf('  SG filter: window=%d, order=%d\n', sg_window, sg_order);
elseif strcmpi(smoothing_method, 'movmean')
    fprintf('  Moving average: window=%d\n', movmean_window);
end
fprintf('Idle threshold: %.2f A\n', idle_thr_cell);
fprintf('Min event duration: %d sec\n', min_duration_sec);
fprintf('Min idle duration before event: %d sec\n', min_idle_duration_sec);
fprintf('\n');
    ...
    for year_idx = 1:length(yearList)
        ...
        for m = 1:length(monthDirs)
            ...
            for f = 1:length(matFiles)
                ...
                if strcmp(type, 'New')
                    all_events = process_rack_data(data, 'Rack01', type, file_info, year_key, month_key, all_events, idle_thr_cell, min_duration_sec, min_idle_duration_sec, smoothing_method, sg_window, sg_order, movmean_window);
                elseif strcmp(type, 'Old')
                    for r_idx = 1:length(rackNames_all)
                        rackName = rackNames_all{r_idx};
                        all_events = process_rack_data(data, rackName, type, file_info, year_key, month_key, all_events, idle_thr_cell, min_duration_sec, min_idle_duration_sec, smoothing_method, sg_window, sg_order, movmean_window);
                    end
                end
...
%% Helper Functions
% =========================================================================
function all_events_out = process_rack_data(data, rackName, type, file_info, year_key, month_key, all_events_in, idle_thr_cell, min_duration_sec, min_idle_duration_sec, smoothing_method, sg_window, sg_order, movmean_window)
    ...
    if strcmp(type, 'New')
        ...
        % 데이터 필터링 적용 (선택된 방법에 따라)
        min_window_size = max(sg_window, movmean_window);
        if length(I_raw) >= min_window_size
            switch lower(smoothing_method)
                case 'sg'
                    % SG 필터 적용 (Savitzky-Golay): 노이즈 제거하되 엣지 유지
                    % 윈도우 크기와 차수 조정 가능
                    I_smooth = sgolayfilt(I_raw, sg_order, sg_window);  % 전류 필터링
                    V_smooth = sgolayfilt(V_raw, sg_order, sg_window);  % 전압 필터링 (위상 맞춤)
                case 'movmean'
                    % 이동평균 필터 적용: 단순 평활화
                    I_smooth = movmean(I_raw, movmean_window);  % 전류 필터링
                    V_smooth = movmean(V_raw, movmean_window);  % 전압 필터링
                otherwise
                    error('Unknown smoothing method: %s. Use ''sg'' or ''movmean''.', smoothing_method);
            end
        else
            % 데이터가 너무 짧으면 필터링 없이 원본 사용
            I_smooth = I_raw;
            V_smooth = V_raw;
        end
        
        % 이벤트 검출: smooth 데이터 사용
        % 시간 정보도 전달하여 휴지 구간 지속 시간 확인 가능하게 함
        [chg_indices, dch_indices] = find_events_simplified(I_smooth, idle_thr_cell, min_duration_sec, t_datetime, min_idle_duration_sec);
...
function [chg_events, dch_events] = find_events_simplified(I_cell, idle_thr, min_duration, t_datetime, min_idle_duration_sec)
    ...
    while i <= n
        ...
        if abs(I_cell(i)) >= idle_thr
            ...
            idle_start_idx = [];  % 휴지 구간 시작 인덱스
            
            % i 이전에서 idle 상태(|I| < idle_thr)의 마지막 지점 찾기
            for k = i-1:-1:1
                if abs(I_cell(k)) < idle_thr
                    event_start = k;  % idle 상태의 마지막 시점부터 저장
                    % 휴지 구간의 시작점 찾기: event_start부터 역순으로 가면서
                    % |I| >= idle_thr인 첫 지점을 찾으면, 그 다음이 휴지 구간 시작
                    idle_start_idx = 1;  % 기본값: 처음부터 휴지 상태
                    for m = event_start-1:-1:1
                        if abs(I_cell(m)) >= idle_thr
                            idle_start_idx = m + 1;  % 휴지 구간 시작
                            break;
                        end
                    end
                    break;
            ...
            if idle_start_idx >= event_start
                % 휴지 구간 시작점이 이벤트 시작점보다 크거나 같으면 스킵
                i = i + 1;
                continue;
            end
            
            % 휴지 구간 지속 시간 계산 (초 단위)
            if isdatetime(t_datetime) && length(t_datetime) >= event_start
                idle_duration_sec = seconds(t_datetime(event_start) - t_datetime(idle_start_idx));
            else
                % 시간 정보가 없으면 샘플 수로 추정 (1초 주기 가정)
                idle_duration_sec = event_start - idle_start_idx;
            end
