...
saveDir = fullfile('D:\JCW\Projects\KEPCO_ESS_Local\FieldData\FieldData_R1s','R1s_Results_OldData');
...
%% Load Old Data
yearList = {'2023'}; % ,'2023'};
rackNames_all = {'Rack01'}; %,'Rack02','Rack03','Rack04','Rack05','Rack06','Rack07','Rack08','Rack09','Rack10'};
    ...
    for m = 1:length(monthDirs)
        ...
        for f = 1:length(matFiles)
            ...
            for rack_idx = 1:length(rackNames)
                ...
                T_amb = rackData.AverageMT_degC; % Ambient Temperature
                
                % Convert to cell units (same as NewData)
                ...
                t_valid = t(2:end); % diff reduces length by 1
                
                % Remove cases where dI or dV is NaN
                validIdx = ~isnan(dI) & ~isnan(dV);
                dI = dI(validIdx);
                dV = dV(validIdx);
                t_valid = t_valid(validIdx);
                ...
                R1s_Results.(dayKey).(rackName).time = t_valid;
                ...
                new_row = table({dayKey}, R1s, R_squared, length(dI), expected_interval, {dI}, {dV}, {t_valid}, ...
        ...
        if ~isempty(monthly_table)
            ...
            if nDays > 0
                ...
                for target_idx = 1:length(SOC_targets)
                    ...
                    sgtitle(sprintf('%s - SOC %d%%-%d%%', month_name, soc_target-1, soc_target+1), 'FontSize', 16);
