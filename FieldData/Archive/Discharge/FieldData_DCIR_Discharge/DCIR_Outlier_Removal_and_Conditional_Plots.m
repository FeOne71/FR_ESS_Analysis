%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% DCIR Outlier Removal and Conditional Plots (Discharge Events)
...
% Modified for discharge events analysis
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

clc; clear; close all;

%% Load original data
dataPath = 'G:\공유 드라이브\Battery Software Lab\Projects\KEPCO_ATB_Lab\ESS_Data_Preprocessing\FieldData_DCIR_Discharge\AutoResults_Discharge\all_dischg_events_current_clustering_all_years.mat';
load(dataPath);

%% Create directories
saveDir = 'G:\공유 드라이브\Battery Software Lab\Projects\KEPCO_ATB_Lab\ESS_Data_Preprocessing\FieldData_DCIR_Discharge\AutoResults_Discharge\OutlierRemoved';
...
function [year_data_all, year_labels, outlier_stats] = collect_year_data(cluster_data, field_name, yearList)
    ...
    outlier_stats.total = total_count;
    outlier_stats.removed = total_count - filtered_total;
end
...
function create_individual_events_plot(year_data, field_name, field_label, year, cluster_label, yearColors, year_idx, figureDir)
    ...
    for k = 1:length(field_name)
        ...
        if ~isempty(event_numbers)
            ...
            title(sprintf('%s %s - %s %s Discharge (%d events)', field_label{k}, year, cluster_label, length(event_numbers)), 'FontSize', 14, 'FontWeight', 'bold');
    ...
    sgtitle(sprintf('DCIR Individual Events - %s %s (Discharge)', year, cluster_label), 'FontSize', 16, 'FontWeight', 'bold');
    saveas(gcf, fullfile(figureDir, sprintf('fig_%s_DCIR_Individual_%s_Discharge.fig', year, cluster_label)));
end

%% Create outlier-removed structure and remove outliers
outlier_removed_struct = global_eventStruct;

fprintf('Removing outliers using 1σ criterion...\n');
for i = 1:length(all_labels)
    label = all_labels{i};
    ...
    for year_idx = 1:length(yearList)
        ...
        % Collect all DCIR values for outlier detection
...
save(fullfile(saveDir, 'all_dischg_events_outlier_removed.mat'), 'outlier_removed_struct');
...
for cluster_idx = 1:length(multi_year_clusters)
    ...
    figure('Name', sprintf('DCIR Box Plot - %s', display_label), 'Position', [100, 100, 1200, 800]);
    for k = 1:length(dcir_fields)
        subplot(2, 3, k);
    ...
    sgtitle(sprintf('DCIR Box Plots - %s (Discharge Events, Outlier Removed)', display_label), 'FontSize', 16, 'FontWeight', 'bold');
    saveas(gcf, fullfile(figureDir, sprintf('fig_DCIR_BoxPlot_%s_Discharge_OutlierRemoved.fig', label)));
    
    % Create DCIR Difference box plots
    figure('Name', sprintf('DCIR Diff Box Plot - %s (Discharge)', display_label), 'Position', [100, 100, 1000, 600]);
    for k = 1:length(dcir_diff_fields)
        ...
        if ~isempty(year_data_all)
            ...
            create_enhanced_boxplot(all_data, group_labels, sprintf('DCIR Diff %s - %s (Discharge)', dcir_diff_labels{k}, display_label), 'DCIR Difference [mΩ]', yearColors, '', outlier_stats);
        end
    end
    sgtitle(sprintf('DCIR Diff Box Plots - %s (Discharge Events, Outlier Removed)', display_label), 'FontSize', 16, 'FontWeight', 'bold');
    saveas(gcf, fullfile(figureDir, sprintf('fig_DCIR_Diff_BoxPlot_%s_Discharge_OutlierRemoved.fig', label)));
    
    % Create DCIR distribution histogram
    figure('Name', sprintf('DCIR Distribution Histogram - %s (Discharge)', display_label), 'Position', [100, 100, 1200, 800]);
    for k = 1:length(dcir_fields)
        subplot(2, 3, k);
        [year_data_all, year_labels, outlier_stats] = collect_year_data(cluster_data, dcir_fields{k}, yearList);
        create_dcir_histogram(year_data_all, year_labels, sprintf('DCIR Distribution %s - %s (Discharge)', dcir_labels{k}, display_label), yearColors, '');
    end
    sgtitle(sprintf('DCIR Distribution Histograms - %s (Discharge Events, Outlier Removed)', display_label), 'FontSize', 16, 'FontWeight', 'bold');
    saveas(gcf, fullfile(figureDir, sprintf('fig_DCIR_Distribution_Histogram_%s_Discharge_OutlierRemoved.fig', label)));
end
