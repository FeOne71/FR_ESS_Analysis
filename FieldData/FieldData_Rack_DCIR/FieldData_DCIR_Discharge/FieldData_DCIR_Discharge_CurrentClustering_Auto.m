%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Field Data DCIR Discharge Current Clustering (Automatic K-means)
% ESS discharging events and resistance analysis (automatic current-based clustering)
...
saveDir  = fullfile('G:\공유 드라이브\Battery Software Lab\Projects\KEPCO_ATB_Lab\ESS_Data_Preprocessing\FieldData_DCIR_Discharge\AutoResults_Discharge');
...
%% Variables
C_nom        = 1024;          % Ah
min_discharge_duration = 30;   % [s] - Discharging duration (30 seconds or more)
...
max_clusters = 3;              % Maximum number of clusters to try
...
for year_idx = 1:length(yearList)
    ...
    for m = 1:length(monthDirs)
        ...
        for f = 1:length(matFiles)
            ...
            bsc_discharge = Raw.Charge;
            dc_power   = Raw.DCPower_kW;
            
%% Step 2: Searching Idle to Discharging Transition
...
%% Step 3: Events Filtering              
            for i = 1:length(idle_to_discharge)
                idx1 = idle_to_discharge(i);
                start_discharge_idx = idx1 + 1;
                
                % Find end point of "Discharging" period
                ...
                idx2 = discharge_end_idx - start_idx + 1;
...
%% Step 4: DCIR Calculation
                ...
                % DCIR calculation based on Discharging end point
...
save(fullfile(saveDir, 'all_dischg_events_current_clustering_all_years.mat'), 'global_eventStruct');
...
% DCIR_All_years;  % Note: This would need to be modified for discharge events

%% Run outlier removal and conditional plotting script
fprintf('\n=== Starting Outlier Removal and Conditional Plotting ===\n');
% DCIR_Outlier_Removal_and_Conditional_Plots;  % Note: This would need to be modified for discharge events 