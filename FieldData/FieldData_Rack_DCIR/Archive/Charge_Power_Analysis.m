...
V_bins = 3.6:0.05:4.2;          % [V] - Fixed voltage bins (3.6V ~ 3.9V, 0.05V intervals)
P_bins = 40:5:80;               % [kW] - Power bins (40kW ~ 80kW, 5kW intervals)
...
for i = 1:length(common_conditions)
    year_list = unique(all_events_data.year);
    fprintf('Condition %d: V_start=%.3f-%.3fV (center=%.3fV), V_fin=%.3f-%.3fV (center=%.3fV), P_avg=%.1fkW (Total: %d events)\n', ...
...
if ~isempty(common_conditions)
    ...
    for y = 1:n_years
        ...
        if ~isempty(year_events_idx)
            ...
            xlabel('Time (s)');
            ylabel('Current (A)');
            title(sprintf('Year %d - Best Common Condition: V_start=%.3f-%.3fV, V_fin=%.3f-%.3fV, P_avg=%.1fkW - Current vs Time (n=%d)', ...
            ...
            xlabel('Time (s)');
            ylabel('Voltage (V)');
            title(sprintf('Year %d - Best Common Condition: V_start=%.3f-%.3fV, V_fin=%.3f-%.3fV, P_avg=%.1fkW - Voltage vs Time (n=%d)', ...
                yearnum, condition.V_start_bin_start, condition.V_start_bin_end, condition.V_fin_bin_start, condition.V_fin_bin_end, condition.P_bin_center, length(year_events_idx)));
            grid on;
            % legend('Location', 'best');
...
if ~isempty(common_conditions)
    ...
    for y = 1:n_years
        ...
        sgtitle(sprintf('Common Condition %d: V_start=%.3f-%.3fV, V_fin=%.3f-%.3fV, P_avg=%.1fkW (Total: %d events)', ...
...
if ~isempty(common_conditions)
    ...
    ylabel('Average R₀ (Ω)');
    title(sprintf('Common Condition %d: V_start=%.3f-%.3fV, V_fin=%.3f-%.3fV, P_avg=%.1fkW (Total: %d events)', ...
        cond_idx, condition.V_start_bin_start, condition.V_start_bin_end, condition.V_fin_bin_start, condition.V_fin_bin_end, condition.P_bin_center, condition.total_events));
    grid on;
    xticks(1:n_periods);
