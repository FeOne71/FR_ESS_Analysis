...
thr = [Cnom*0.1, Cnom*0.1]; % Idle threshold [charge, discharge] (A)
dI_chg = Cnom * 0.08;         % Charge current change threshold (A)        I(i+1) - I(i) > 16.64A
dI_dch = Cnom * 0.08;         % Discharge current change threshold (A)     I(i+1) - I(i) < -25.6A
ddI = 1;                       % Instantaneous current change threshold (A)
te = 4;   % Moving average window (6 samples for 1Hz data = 6 seconds)
dt = 2;   % Peak duration (1 sample for 1Hz data = 1 second)
    ...
    for m = 1:length(monthDirs)
        ...
        for f = 1:length(matFiles)
            ...
            for rack_idx = 1:length(rackNames)
                ...
                for i = 1:N
                    for m = 0:te
                        I_filt(i) = I_filt(i) + calc_dI_dt(i+m);
                    end
                    I_filt(i) = I_filt(i)/(te+1);
                end
...
for r = 1:length(rackNames_all)
    ...
    for yi = 1:length(yearList)
        ...
        if ~isempty(chg_vals_cleaned)
            ...
            % Re-separate cleaned data by year for plotting
            ...
            % Plot each year's cleaned data
            for yi = 1:length(yearList)
                if ~isempty(chg_vals_cleaned_by_year{yi})
                    h = histfit(chg_vals_cleaned_by_year{yi}, 30, 'normal');
        ...
        legend(legend_handles, yearList, 'Location', 'northeast');
        ...
        if ~isempty(dch_vals_cleaned)
            ...
            for yi = 1:length(yearList)
                if ~isempty(dch_vals_cleaned_by_year{yi})
                    h = histfit(dch_vals_cleaned_by_year{yi}, 30, 'normal');
                    h(1).EdgeColor = yearColors{yi};
                    h(1).FaceColor = yearColors{yi};
                    h(1).FaceAlpha = 0.7;  % Same transparency as charge
                    h(2).Color = yearColors{yi};
                    h(2).LineWidth = 3;
                    legend_handles = [legend_handles, h(1)];
    ...
    if ~isempty(chg_vals_cleaned)
        ...
        if ~isempty(chg_vals_cleaned)
            ...
            title(sprintf('Charge Peaks Boxplot - %s (3σ outliers removed, Mean shown)', rackName));
            set(findall(gcf,'-property','FontSize'),'FontSize',20);
            set(findall(gcf,'-property','interpreter'),'interpreter','tex');
            set(findall(gcf,'-property','ticklabelinterpreter'),'ticklabelinterpreter','tex');
    ...
    if ~isempty(dch_vals_cleaned)
        ...
        if ~isempty(dch_vals_cleaned)
            ...
            title(sprintf('Discharge Peaks Boxplot - %s (3σ outliers removed, Mean shown)', rackName));
            set(findall(gcf,'-property','FontSize'),'FontSize',20);
            set(findall(gcf,'-property','interpreter'),'interpreter','tex');
            set(findall(gcf,'-property','ticklabelinterpreter'),'ticklabelinterpreter','tex');
