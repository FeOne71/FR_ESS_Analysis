...
for i_folder = 1:length(folders_driving)
    load(['../../Drive/Folder' folders_driving{i_folder} '/Raw.mat'])
                ...
                if (I_filt(i+1)-I_filt(i))>ddI
                    ...
                    if flag==1
                        if z==1
                            ...
                            % Additional condition that discards a new peak if a sufficient amount of time (dt=1s) is not passed from the
                            % beginning of the last peak (in this way the same peak cannot be saved twice in case of numerical errors) 
