%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Rack Data DCIR Charge - Onori Method WITHOUT Moving Average Filter
% 1초 샘플링 데이터, 원본 전류 데이터로 직접 피크 감지 및 DCIR 계산
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
saveDir  = fullfile('G:\공유 드라이브\Battery Software Lab\Projects\KEPCO_ATB_Lab\ESS_Data_Preprocessing\FieldData\FieldData_Rack_DCIR\DCIR_Charge_Onori_NotFiltered');
...
dI  = C_nom_cell * 0.2;    % Current change threshold after 4s (A)
    ...
    for m = 1:length(monthDirs)
        ...
        for f = 1:length(matFiles)
            ...
            for rack_idx = 1:length(rackNames)
                ...
                % Driving Peaks Identification (이동평균 없이 원본 데이터 사용)
                PeakTime = {};
                PeakCurrent = {};
                PeakVoltage = {};
                z = 1;
...
if ~isempty(all_peak_times)
    ...
    title('Rack01 Voltage Data with Detected Peaks (No Filter)');
    ...
    set(gca, 'fontsize', Fontsize);
    set(gca, 'ticklabelinterpreter', 'tex');
    title('Rack01 Current Data with Detected Peaks (No Filter)');
    
    % Calculate rack01 peaks for the selected day
    rack01_peaks_for_day = length(PeakTime);
    
    sgtitle(sprintf('Date: %s, Peaks: %d (No Moving Average Filter)', formatted_date, rack01_peaks_for_day), 'FontSize', 16);
    
    fig_filename = fullfile(saveDir, 'Onori_Method_NoFilter_Visualization.fig');
    saveas(gcf, fig_filename);
    fprintf('Figure saved to: %s\n', fig_filename);
    
    fprintf('Visualization complete: %d peaks detected (No Filter).\n', length(PeakTime));
else
    fprintf('No peaks detected.\n');
end

save(fullfile(saveDir, 'all_chg_events_onori_nofilter_all_years.mat'), 'global_eventStruct');
