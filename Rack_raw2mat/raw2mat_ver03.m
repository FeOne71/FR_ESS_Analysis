%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Convert KIMJ ESS CSV file to MAT file - 간결한 버전
% 모든 데이터를 배열로 저장하는 방식
...
% 데이터 저장 경로 설정
data_dir = 'D:\JCW\KENTECH\Projects\KEPCO\Data';
years    = {'202106_KIMJ','202206_KIMJ','202306_KIMJ'};  % 연도별 폴더
save_dir = 'D:\JCW\KENTECH\Projects\KEPCO\ESS_Data_Preprocessing';
    ...
    for day = 1:last_day
        ...
        for i = 1:length(online_files)
            try
                ...
                % 필요한 변수 목록
                vars_needed = {'DCCurrent_A_', 'HighestDCCurrent_A_', 'LowestDCCurrent_A_'};
            ...
            if sum(bsc_mask) > 0
                ...
                if length(Raw.sync_Time) >= 2 && length(Raw.Plc_Time) >= 2
                    try
                        ...
                        % 배터리 온도 보간
                        if ~isempty(Raw.Plc_Battery_Temperature)
                            Raw.Plc_Battery_Temperature_sync = interp1(plc_time_num, Raw.Plc_Battery_Temperature, Raw.sync_Time_num, 'linear', 'extrap');
                        end
                        
                        % 습도 보간
                        if ~isempty(Raw.Plc_Humidity)
                            Raw.Plc_Humidity_sync = interp1(plc_time_num, Raw.Plc_Humidity, Raw.sync_Time_num, 'linear', 'extrap');
                        end
                        
                        % 주변 온도 보간
                        if ~isempty(Raw.Plc_Ambient_Temperature)
                            Raw.Plc_Ambient_Temperature_sync = interp1(plc_time_num, Raw.Plc_Ambient_Temperature, Raw.sync_Time_num, 'linear', 'extrap');
                        end
                    catch
                        % 오류 무시하고 계속 진행
                    end
