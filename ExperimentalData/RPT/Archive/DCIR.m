...
%RPT data load
original_mat_path = 'D:\JCW\Projects\KEPCO_ESS_Local\ExperimentalData\RPT\250423 RPT-Aging_Li_1.mat';
...
%% Figure plotting (commented out - original bar chart)
% figure(1);
% hold on;
% 
...
% 
% for i = 1:n
%     SOC(i) = chg_data_DCIR_selected(i).SOC;  % Actual SOC value
%     dR(i,:) = chg_data_DCIR_selected(i).dR;  % 1x3 vector
% end
% 
% % Uniformly distributed fake x-axis positions (1, 2, 3, ...)
% x_pos = 1:n;
% 
% % Draw bar chart
% bar(x_pos, dR, 'stacked');
% 
% % Set x-axis ticks → Display SOC values
% xticks(x_pos);
% [~, file_name, ~] = fileparts(original_mat_path);  % Extract filename without extension
...
% 
% for i = 1:n
%     SOC(i) = dis_data_DCIR_selected(i).SOC;  % Actual SOC value
...
% 
% % Set x-axis ticks → Display SOC values
% xticks(x_pos);[~, file_name, ~] = fileparts(original_mat_path);  % Extract filename without extension
% title(file_name, 'Interpreter', 'none','FontSize',14)
% xticklabels(arrayfun(@(x) sprintf('%.2f', x), SOC, 'UniformOutput', false));
% xtickangle(45);  % Rotate x-axis labels
% 
...
%% New visualization (scatter plot with colorbar)
% Create figure
...
if ~isempty(chg_data_DCIR_selected)
    ...
    R_chg = zeros(n_chg, 3);      % Point-in-time resistance (1s, 5s, 25s)
    ...
    % Plot charge data (1s, 5s, 25s point-in-time resistance)
    nexttile(1);
    hold on;
    scatter(SOC_chg, R_chg(:,1), 50, 'b', 'filled', 'DisplayName', '1s resistance');
    scatter(SOC_chg, R_chg(:,2), 50, 'g', 'filled', 'DisplayName', '5s resistance');
    scatter(SOC_chg, R_chg(:,3), 50, 'r', 'filled', 'DisplayName', '25s resistance');
    xlabel('State of Charge (%)', 'FontSize', 12);
    ylabel('DCIR (Ω)', 'FontSize', 12);
    title('Charge DCIR vs SOC (Point-in-time Resistance)', 'FontSize', 14);
    grid on;
...
if ~isempty(dis_data_DCIR_selected)
    ...
    R_dis = zeros(n_dis, 3);      % Point-in-time resistance (1s, 5s, 25s)
    ...
    % Plot discharge data (1s, 5s, 25s point-in-time resistance)
    nexttile(2);
    hold on;
    scatter(SOC_dis, R_dis(:,1), 50, 'b', 'filled', 'DisplayName', '1s resistance');
    scatter(SOC_dis, R_dis(:,2), 50, 'g', 'filled', 'DisplayName', '5s resistance');
    scatter(SOC_dis, R_dis(:,3), 50, 'r', 'filled', 'DisplayName', '25s resistance');
    xlabel('Depth of Discharge (%)', 'FontSize', 12);
    ylabel('DCIR (Ω)', 'FontSize', 12);
    title('Discharge DCIR vs SOC (Point-in-time Resistance)', 'FontSize', 14);
    grid on;
    legend('Location', 'best');
    hold off;
end 
...
%% Save results
% Save new figure
savefig(fig, fullfile(save_path, sprintf('%s_DCIR_SOC.fig', char(base_filename))));
