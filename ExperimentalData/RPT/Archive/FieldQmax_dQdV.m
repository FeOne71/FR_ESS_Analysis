...
for date_idx = 1:length(dates)
    ...
    for k = 1:length(Charge_dQdV_data)
        if ~isempty(Charge_dQdV_data(k).V_dQdV)
            plot(ax1, Charge_dQdV_data(k).V_dQdV, Charge_dQdV_data(k).dQdV, '-', ...
    ...
    for k = 1:length(Charge_dQdV_data)
        if ~isempty(Charge_dQdV_data(k).I_raw)
            plot(ax2, Charge_dQdV_data(k).t_raw, Charge_dQdV_data(k).I_raw, '-', ...
                'Color', colors(k,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Segment %d', k));
        end
    end
    ...
    for k = 1:length(Charge_dQdV_data)
        if ~isempty(Charge_dQdV_data(k).V_raw)
            plot(ax3, Charge_dQdV_data(k).t_raw, Charge_dQdV_data(k).V_raw, '-', ...
                'Color', colors(k,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Segment %d', k));
        end
    end
    title(ax3, 'Charge Voltage');
    xlabel(ax3, 'Time');
    ...
    for k = 1:length(Charge_dQdV_data)
        if ~isempty(Charge_dQdV_data(k).t_raw) && ~isempty(Charge_dQdV_data(k).SOC_raw)
            plot(ax4, Charge_dQdV_data(k).t_raw, Charge_dQdV_data(k).SOC_raw, '-', ...
                'Color', colors(k,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Segment %d', k));
        end
    end
    ...
    for k = 1:length(Discharge_dQdV_data)
        if ~isempty(Discharge_dQdV_data(k).V_dQdV)
            plot(ax5, Discharge_dQdV_data(k).V_dQdV, Discharge_dQdV_data(k).dQdV, '-', ...
    ...
    for k = 1:length(Discharge_dQdV_data)
        if ~isempty(Discharge_dQdV_data(k).I_raw)
            plot(ax6, Discharge_dQdV_data(k).t_raw, Discharge_dQdV_data(k).I_raw, '-', ...
                'Color', colors_dischg(k,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Segment %d', k));
        end
    end
    ...
    for k = 1:length(Discharge_dQdV_data)
        if ~isempty(Discharge_dQdV_data(k).V_raw)
            plot(ax7, Discharge_dQdV_data(k).t_raw, Discharge_dQdV_data(k).V_raw, '-', ...
                'Color', colors_dischg(k,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Segment %d', k));
        end
    end
    title(ax7, 'Discharge Voltage');
    xlabel(ax7, 'Time');
    ...
    for k = 1:length(Discharge_dQdV_data)
        if ~isempty(Discharge_dQdV_data(k).t_raw) && ~isempty(Discharge_dQdV_data(k).SOC_raw)
            plot(ax8, Discharge_dQdV_data(k).t_raw, Discharge_dQdV_data(k).SOC_raw, '-', ...
                'Color', colors_dischg(k,:), 'LineWidth', 1.5, 'DisplayName', sprintf('Segment %d', k));
        end
    end
...
for date_idx = 1:length(dates)
    ...
    for k = 1:length(Charge_data)
        if ~isempty(Charge_data(k).V_dQdV)
            plot(ax1, Charge_data(k).V_dQdV, Charge_data(k).dQdV, '-', ...
...
for date_idx = 1:length(dates)
    ...
    for k = 1:length(Charge_data)
        if ~isempty(Charge_data(k).I_raw)
            ...
            plot(ax2, t_normalized, Charge_data(k).I_raw, '-', ...
                'Color', year_colors(date_idx,:), 'LineWidth', 1.5, ...
                'DisplayName', sprintf('%s Seg%d', date_str, k));
        end
    end
...
for date_idx = 1:length(dates)
    ...
    for k = 1:length(Charge_data)
        if ~isempty(Charge_data(k).V_raw)
            % 시간을 0부터 시작하도록 normalize (초 단위)
            t_normalized = seconds(Charge_data(k).t_raw - Charge_data(k).t_raw(1));
            plot(ax3, t_normalized, Charge_data(k).V_raw, '-', ...
                'Color', year_colors(date_idx,:), 'LineWidth', 1.5, ...
                'DisplayName', sprintf('%s Seg%d', date_str, k));
        end
    end
end
title(ax3, 'Charge Voltage: All Years');
xlabel(ax3, 'Time [s]');
ylabel(ax3, 'Voltage [V]');
...
for date_idx = 1:length(dates)
    ...
    for k = 1:length(Discharge_data)
        if ~isempty(Discharge_data(k).V_dQdV)
            plot(ax1, Discharge_data(k).V_dQdV, Discharge_data(k).dQdV, '-', ...
                'Color', year_colors(date_idx,:), 'LineWidth', 1.5, ...
                'DisplayName', sprintf('%s Seg%d', date_str, k));
        end
    end
...
for date_idx = 1:length(dates)
    ...
    for k = 1:length(Discharge_data)
        if ~isempty(Discharge_data(k).I_raw)
            ...
            plot(ax2, t_normalized, Discharge_data(k).I_raw, '-', ...
                'Color', year_colors(date_idx,:), 'LineWidth', 1.5, ...
                'DisplayName', sprintf('%s Seg%d', date_str, k));
        end
    end
...
for date_idx = 1:length(dates)
    ...
    for k = 1:length(Discharge_data)
        if ~isempty(Discharge_data(k).V_raw)
            % 시간을 0부터 시작하도록 normalize (초 단위)
            t_normalized = seconds(Discharge_data(k).t_raw - Discharge_data(k).t_raw(1));
            plot(ax3, t_normalized, Discharge_data(k).V_raw, '-', ...
                'Color', year_colors(date_idx,:), 'LineWidth', 1.5, ...
                'DisplayName', sprintf('%s Seg%d', date_str, k));
        end
    end
end
title(ax3, 'Discharge Voltage: All Years');
xlabel(ax3, 'Time [s]');
ylabel(ax3, 'Voltage [V]');
legend(ax3, 'Location', 'best', 'NumColumns', 2);
