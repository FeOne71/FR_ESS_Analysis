...
for i = 1:length(channels)
    ...
    for c = 1:length(cyc_fields)
        ...
        for r = 1:length(target_crates)
            ...
            %% --- [6] AEKF Hybrid Features & SOH_p (t_raw, V_raw, I_raw) ---
            % Check if raw time exists, otherwise synthesize dt=1
            if isfield(s_chg, 't_raw'), t_chg = s_chg.t_raw; else, t_chg = (1:length(s_chg.V_raw))'; end
            if isfield(s_dch, 't_raw'), t_dch = s_dch.t_raw; else, t_dch = (1:length(s_dch.V_raw))'; end
            
            % Nominal capacity for this segment
            Q_cap_Ah = Q_0;
            if isnan(Q_cap_Ah) || Q_cap_Ah == 0, Q_cap_Ah = 64; end % fallback to typical Np=1 cell capacity
            
            % Explicitly cast to double to prevent EKF type errors
            v_chg_d = double(s_chg.V_raw); i_chg_d = double(s_chg.I_raw); t_chg_d = double(t_chg);
            v_dch_d = double(s_dch.V_raw); i_dch_d = double(s_dch.I_raw); t_dch_d = double(t_dch);
            
            % AEKF SOH_p Charge
            try
                [SOH_p_chg, ~, ~, ~, ~] = Physics_Prior_Calculator(v_chg_d, i_chg_d, t_chg_d, 0.05, Q_cap_Ah, path_master_ruler);
                [~, ~, ECM_chg] = AEKF_SOC_Estimator(v_chg_d, i_chg_d, t_chg_d, 0.05, Q_cap_Ah, MasterRulers.(ch).Fresh_OCV_Charge.Q/max(MasterRulers.(ch).Fresh_OCV_Charge.Q), MasterRulers.(ch).Fresh_OCV_Charge.V_grid);
                R0_chg = ECM_chg.R0; R1_chg = ECM_chg.R1; C1_chg = ECM_chg.C1;
            catch e
                SOH_p_chg = NaN; R0_chg = NaN; R1_chg = NaN; C1_chg = NaN;
                fprintf('AEKF failed for chg %s %s: %s\n', ch, crate_label, e.message);
            end

            % AEKF SOH_p Discharge
            try
                [SOH_p_dch, ~, ~, ~, ~] = Physics_Prior_Calculator(v_dch_d, -abs(i_dch_d), t_dch_d, 0.95, Q_cap_Ah, path_master_ruler);
                [~, ~, ECM_dch] = AEKF_SOC_Estimator(v_dch_d, -abs(i_dch_d), t_dch_d, 0.95, Q_cap_Ah, MasterRulers.(ch).Fresh_OCV_Charge.Q/max(MasterRulers.(ch).Fresh_OCV_Charge.Q), MasterRulers.(ch).Fresh_OCV_Charge.V_grid);
                R0_dch = ECM_dch.R0; R1_dch = ECM_dch.R1; C1_dch = ECM_dch.C1;
            catch e
                SOH_p_dch = NaN; R0_dch = NaN; R1_dch = NaN; C1_dch = NaN;
            end

            %% --- Collect row (29 features including AEKF) ---
            cnt = cnt + 1;
            ...
            data_X(cnt,:) = [dQ_chg, dQ_dch, ...
                             ...
                             SOH_p_chg, R0_chg, R1_chg, C1_chg, SOH_p_dch, R0_dch, R1_dch, C1_dch];
        end
    end
end
