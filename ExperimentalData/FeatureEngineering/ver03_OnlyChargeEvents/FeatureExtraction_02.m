...
for p = 1:length(paramSets)
    ...
    if isempty(matFiles)
        fprintf('  WARNING: No raw event files found in %s\n', inputDir);
        continue;
    end
        ...
        for s = 1:length(socs)
            ...
            for p = 1:length(profs)
                ...
                for d_idx = 1:length(dt_list)
                    ...
                    fname = sprintf('%s_%ds', rPrefix, dt_target);
                    rawEvents.(ctName).(sName).(pName).(evtName).(fname) = R;
                end
...
fprintf('All results saved under: %s\n', baseResultsDir);
