...
%% Load original data
dataPath = 'G:\공유 드라이브\Battery Software Lab\0_Group Meeting\개인별_미팅자료\정철원\Materials\ESS_Data_Preprocessing\FieldData_DCIR_Charge\AutoResults\all_chg_events_current_clustering_all_years.mat';
load(dataPath);

%% Create directories
saveDir = 'G:\공유 드라이브\Battery Software Lab\0_Group Meeting\개인별_미팅자료\정철원\Materials\ESS_Data_Preprocessing\FieldData_DCIR_Charge\AutoResults\OutlierRemoved';
...
function create_individual_events_plot(year_data, field_name, field_label, year, cluster_label, yearColors, year_idx, figureDir)
    ...
    for k = 1:length(field_name)
        ...
        if ~isempty(event_numbers)
            ...
            title(sprintf('%s %s - %s %s (%d events)', field_label{k}, year, cluster_label, length(event_numbers)), 'FontSize', 14, 'FontWeight', 'bold');
    ...
    sgtitle(sprintf('DCIR Individual Events - %s %s', year, cluster_label), 'FontSize', 16, 'FontWeight', 'bold');
...
for cluster_idx = 1:length(multi_year_clusters)
    ...
    figure('Name', sprintf('DCIR Diff Box Plot - %s', display_label), 'Position', [100, 100, 1000, 600]);
