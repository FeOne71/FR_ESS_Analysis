...
if ~isempty(summaryTable) && height(summaryTable) > 0
    ...
    for r_idx = 1:length(availableResistanceVars)
        ...
        for et_idx = 1:length(uniqueEventTypes)
            ...
            fig = figure('Name', sprintf('Boxplot: Capacity_C3 vs %s - %s Events', rLabel, eventType), ...
                        'Position', [100 + r_idx*30 + et_idx*20, 100, 1600, 900], 'Visible', 'on');
            ...
            for dc_idx = 1:length(uniqueDCs)
                ...
                % Filter data for this DC and EventType
                dcMask = strcmp(eventData.DC_Profile, dcProfile) & ...
                         ~isnan(eventData.Capacity_C3) & ...
                         ~isnan(eventData.(rVar));
                
                dcData = eventData(dcMask, :);
                
                if height(dcData) == 0
                    text(0.5, 0.5, 'No Data', 'HorizontalAlignment', 'center', ...
                         'VerticalAlignment', 'middle', 'FontSize', 14, ...
                         'Color', 'red', 'Units', 'normalized');
                    title(sprintf('%s - %s', dcProfile, eventType), 'FontSize', 11, 'FontWeight', 'bold');
                    continue;
                end
            ...
            % Overall title
            sgtitle(sprintf('Capacity_C3 vs %s (%s Events)\nBoxplot by DC Profile', ...
                           rLabel, eventType), 'FontSize', 14, 'FontWeight', 'bold');
            
            % Save figure
            savePath = fullfile(figuresDir, sprintf('Boxplot_Capacity_vs_%s_%s.fig', rVar, eventType));
            saveas(fig, savePath);
            fprintf('  Saved: Boxplot_Capacity_vs_%s_%s.fig\n', rVar, eventType);
            close(fig);
        end
    end
