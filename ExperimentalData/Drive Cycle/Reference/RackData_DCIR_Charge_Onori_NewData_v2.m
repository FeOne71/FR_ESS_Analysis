%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Rack Data DCIR Charge - New Logic v2 (idle -> peak current)
% 2023-2025 Data
% Peak detection: NewLogic v2 (idle -> current peak detection)
% 1초 샘플링 데이터, 전류 최대값 도달 시점 피크 검출 로직
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
saveDir = fullfile('D:\JCW\Projects\KEPCO_ESS_Local\FieldData\FieldData_Rack_DCIR','TotalPeak_NewData_v2');
...
dI_chg = C_nom_cell * 0.15; % Charge: ΔI > dI_chg (A)
dI_dch = C_nom_cell * 0.25; % Discharge: ΔI < -dI_dch (A)
te = 6;  % Moving average window: ±6 seconds (1초 샘플링 기준)
dt = 2; 
% ddI = 1;
...
for year_idx = 1:length(yearList)
    ...
    for m = 1:length(monthDirs)
        ...
        for f = 1:length(matFiles)
            ...
            % Current derivative calculation
            ...
            % New Peak Detection Logic v2 - 전류 최대값 도달 시점 검출
            PeakTime = {};
            PeakCurrent = {};
            PeakVoltage = {};
            PeakSoC = {};
            ...
            %% Charge Peak Detection Logic v2 - 전류 최대값 도달 시점

            for i = 1:(length(I)-10)  % 충분한 여유 확보
                % 1단계: 휴지 상태 확인
                if I(i) > -thr(1) && I(i) < thr(1)
                    % 2단계: 전류 증가 시작 확인
                    if filt_dI_dt(i) > 0  % dIdt 기반 트리거
                        % 3단계: 전류 최대값 도달 시점 찾기 (dI/dt <= 0 바로 전 시점)
                        peak_idx = i;
                        for j = i+1:min(i+3, length(I)-1)  % 최대 50초까지 탐색
                            if dI_dt(j) <= 0  % 전류가 더 이상 증가하지 않는 시점
                                peak_idx = j;
                                break;
                            end
                        end
                        ...
                        % 4단계: 총 전류 변화량 확인
                        dI_total = I(peak_idx) - I(i);
                        if dI_total > dI_chg
                            % 5단계: 전압 범위 필터링 (선택사항)
                            % if V(i) >= 3.5  % 필요시 활성화
                            % 피크 저장 (시작점과 전류 최대값 도달 시점)
                            PeakTime{z} = [t(i), t(peak_idx)];
                            PeakCurrent{z} = [I(i), I(peak_idx)];
                            PeakVoltage{z} = [V(i), V(peak_idx)];
                            PeakSoC{z} = soc(i);
                            PeakTemp{z} = T_batt(i);
                            z = z + 1;
                            % end
            ...
            % Resistance Computation - CHARGE (save full segment, keep 2-point pair)
            last_end_idx_chg = 0;  % duplicate guard
            for i = 1:length(PeakTime)
                start_idx_tmp = find(t >= PeakTime{i}(1), 1);
                end_idx_tmp = find(t >= PeakTime{i}(end), 1);
                if isempty(start_idx_tmp) || isempty(end_idx_tmp) || end_idx_tmp <= start_idx_tmp
                    continue;
                end
                % skip duplicates/overlaps (same end or overlapping start)
                if end_idx_tmp == last_end_idx_chg || start_idx_tmp <= last_end_idx_chg
                    continue;
                end
                ...
                eventStruct.(evtName).start_idx = start_idx_tmp;
                eventStruct.(evtName).end_idx = end_idx_tmp;
                eventStruct.(evtName).charge_duration = PeakTime{i}(end) - PeakTime{i}(1);

                % full sequences
                eventStruct.(evtName).t_seq = Raw.Date_Time(start_idx_tmp:end_idx_tmp);
                eventStruct.(evtName).I_seq = Raw.DCCurrent(start_idx_tmp:end_idx_tmp);
                eventStruct.(evtName).V_seq = Raw.CVavg(start_idx_tmp:end_idx_tmp);
                eventStruct.(evtName).P_seq = Raw.DCPower(start_idx_tmp:end_idx_tmp);
                eventStruct.(evtName).avg_current = mean(eventStruct.(evtName).I_seq);
                % 2-point pairs used for DCIR
                eventStruct.(evtName).I_pair = PeakCurrent{i};
                eventStruct.(evtName).V_pair = PeakVoltage{i};
                % Peak 저항 저장
                eventStruct.(evtName).PeakChgR = dcir_val;

                last_end_idx_chg = end_idx_tmp;
            end

            % Debug peak detection
            if ~isempty(PeakTime)
                avg_voltage = mean(cellfun(@(x) x(1), PeakVoltage));
                avg_duration = mean(cellfun(@(x) x(end) - x(1), PeakTime));
                fprintf('Rack %s: %d charge peaks detected (Voltage: %.2fV, Avg Duration: %.1fs)\n', rackName, length(PeakTime), avg_voltage, avg_duration);
            else
                fprintf('Rack %s: No charge peaks detected\n', rackName);
            end

            %% Discharge Peak Detection Logic v2 - 전류 최소값 도달 시점
            PeakTimeDis = {}; PeakCurrentDis = {}; PeakVoltageDis = {}; PeakSoCDis = {}; PeakTempDis = {}; zd = 1;
            ...
            for i = 1:(length(I)-10)  % 충분한 여유 확보
                ...
                if I(i) > -thr(2) && I(i) < thr(2)
                    % 2단계: 전류 감소 시작 확인
                    if filt_dI_dt(i) < 0  % dIdt 기반 트리거
                        % 3단계: 전류 최소값 도달 시점 찾기 (dI/dt >= 0 바로 전 시점)
                        peak_idx = i;
                        for j = i+1:min(i+3, length(I)-1)  % 최대 50초까지 탐색
                            if dI_dt(j) >= 0  % 전류가 더 이상 감소하지 않는 시점
                                peak_idx = j;
                                break;
                            end
                        end
                        ...
                        if dI_total < -dI_dch
                            % 5단계: 전압 범위 필터링 (선택사항)
                            % if V(i) <= 3.9  % 필요시 활성화
                            % 피크 저장 (시작점과 전류 최소값 도달 시점)
                            PeakTimeDis{zd} = [t(i), t(peak_idx)];
                            PeakCurrentDis{zd} = [I(i), I(peak_idx)];
                            PeakVoltageDis{zd} = [V(i), V(peak_idx)];
                            PeakSoCDis{zd} = soc(i);
            ...
            % Resistance Computation - DISCHARGE (save full segment, keep 2-point pair)
            last_end_idx_dch = 0;  % duplicate guard
            for i = 1:length(PeakTimeDis)
                start_idx_tmp = find(t >= PeakTimeDis{i}(1), 1);
                end_idx_tmp = find(t >= PeakTimeDis{i}(end), 1);
                if isempty(start_idx_tmp) || isempty(end_idx_tmp) || end_idx_tmp <= start_idx_tmp
                    continue;
                end
                % skip duplicates/overlaps
                if end_idx_tmp == last_end_idx_dch || start_idx_tmp <= last_end_idx_dch
                    continue;
                end
                ...
                eventStruct.(evtName).start_idx = start_idx_tmp;
                eventStruct.(evtName).end_idx = end_idx_tmp;
                eventStruct.(evtName).charge_duration = PeakTimeDis{i}(end) - PeakTimeDis{i}(1);
                ...
                eventStruct.(evtName).avg_current = mean(eventStruct.(evtName).I_seq);
                % 2-point pairs used for DCIR
                eventStruct.(evtName).I_pair = PeakCurrentDis{i};
                eventStruct.(evtName).V_pair = PeakVoltageDis{i};
                eventStruct.(evtName).PeakDisR = dis_val;

                last_end_idx_dch = end_idx_tmp;
            end

            % Debug discharge peak detection
            if ~isempty(PeakTimeDis)
                avg_voltage_dis = mean(cellfun(@(x) x(1), PeakVoltageDis));
                avg_duration_dis = mean(cellfun(@(x) x(end) - x(1), PeakTimeDis));
                fprintf('Rack %s: %d discharge peaks detected (Voltage: %.2fV, Avg Duration: %.1fs)\n', rackName, length(PeakTimeDis), avg_voltage_dis, avg_duration_dis);
            else
...
save(fullfile(saveDir, sprintf('Peak_NewData_v2_%s_total.mat', yearStr)), 'global_eventStruct');
