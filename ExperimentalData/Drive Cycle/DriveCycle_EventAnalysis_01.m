...
if debug_mode
    if ~isempty(cycleTypes)
        ...
        fprintf('\n[DEBUG MODE] Processing only first cycle: %s\n', cycleTypes{1});
        fprintf('  (All channels/SOC use identical drive cycle, no need to process all cycles)\n');
    end
end
...
for cycleIdx = 1:length(cycleTypes)
    ...
    for ch_idx = 1:length(channels)
        ...
        for soc_idx = 1:length(soc_levels)
            ...
            for prof_idx = 1:length(profiles)
                ...
                % Debug: Print transition type counts
                if num_charging_transitions > 0 || num_discharging_transitions > 0
                    fprintf('    DEBUG %s: %d charging transitions, %d discharging transitions detected\n', ...
                        profile_name, num_charging_transitions, num_discharging_transitions);
                end
    ...
    % Print filtering statistics for this cycle
    fprintf('\n');
    fprintf('========================================\n');
    fprintf('=== Filtering Statistics: %s ===\n', cycleType);
    fprintf('========================================\n');
    
    % Get all channels from filterStats
    channelNames = fieldnames(filterStats);
    ...
    for chIdx = 1:length(channelNames)
        ...
        for socIdx = 1:length(socLevels)
            ...
            fprintf('\n  SOC: %s\n', socLevel);
            fprintf('  %-6s | %-15s | %-20s | %-20s | %-20s | %-15s | %-15s | %-15s | %-15s | %-20s\n', ...
                'DC', 'Total Transitions', 'Rejected Short Duration', 'Rejected Direction', 'Rejected Too Short Window', ...
                'Rejected I Std', 'Rejected CV', 'Rejected P Std', 'Rejected Too Short After Cut');
            fprintf('  ------|-----------------|----------------------|----------------------|----------------------|-----------------|-----------------|-----------------|----------------------\n');
            ...
            for dcIdx = 1:length(dcProfiles)
                ...
                totalTransitions = stats.total_transitions;
                rejectedShortDuration = stats.rejected_short_duration;
                rejectedDirection = stats.rejected_direction;
                rejectedTooShortWindow = stats.rejected_too_short_window;
                rejectedIStd = stats.rejected_I_std;
                rejectedCV = stats.rejected_CV;
                rejectedPStd = stats.rejected_P_std;
                rejectedTooShortAfterCut = stats.rejected_too_short_after_cut;
                
                fprintf('  %-6s | %15d | %20d | %20d | %20d | %15d | %15d | %15d | %20d\n', ...
                    dcProfile, totalTransitions, rejectedShortDuration, rejectedDirection, rejectedTooShortWindow, ...
                    rejectedIStd, rejectedCV, rejectedPStd, rejectedTooShortAfterCut);
            end
        end
    end
    
    % Detailed debug summary for ch09, ch15, ch16 (charging events only)
    fprintf('\n\n========================================\n');
    fprintf('=== Charging Event Debug Summary: %s ===\n', cycleType);
    fprintf('========================================\n');
    fprintf('Channels: ch09, ch15, ch16 only\n');
    fprintf('Current Filtering Parameters:\n');
    fprintf('  Min Duration: %.1f s\n', min_duration_chg);
    fprintf('  Max P Std: %.3f kW\n', max_P_std_chg);
    fprintf('  Max I Std: %.2f A\n', max_I_std_chg);
    fprintf('  Max CV: %.1f%%\n', max_CV_chg*100);
    fprintf('  Stable Window: %.1f-%.1f s from driving start\n', 3.0, 10.0);
    fprintf('  Min Duration After Cut: %.1f s\n', 5.0);
    fprintf('\n');
    
    debugChannels = {'ch9_', 'ch15_', 'ch16_'};
    for debugChIdx = 1:length(debugChannels)
        debugChPattern = debugChannels{debugChIdx};
        
        % Find matching channels
        matchingChannels = {};
        for chIdx = 1:length(channelNames)
            if contains(channelNames{chIdx}, debugChPattern)
                matchingChannels{end+1} = channelNames{chIdx};
            end
        end
        ...
        for matchIdx = 1:length(matchingChannels)
            channelName = matchingChannels{matchIdx};
            
            fprintf('\n--- Channel: %s (Charging Events Only) ---\n', channelName);
            
            % Aggregate statistics across all SOC and DC profiles
            totalChgTransitions = 0;
            ...
            for socIdx = 1:length(socLevels)
                ...
                if ~isfield(filterStats, channelName) || ~isfield(filterStats.(channelName), socLevel)
                    continue;
                end
                ...
                for dcIdx = 1:length(dcProfiles)
                    ...
                    stats = filterStats.(channelName).(socLevel).(dcProfile);
                    totalChgTransitions = totalChgTransitions + stats.total_transitions;
            ...
            totalRejections = totalRejectedShortDuration + totalRejectedDirection + totalRejectedTooShortWindow + ...
                totalRejectedIStd + totalRejectedCV + totalRejectedPStd + totalRejectedTooShortAfterCut;
            
            if totalChgTransitions == 0
                fprintf('  No charging transitions detected.\n');
                continue;
            end
            
            fprintf('  Total Charging Transitions: %d\n', totalChgTransitions);
            fprintf('  Total Rejections: %d (%.1f%%)\n', totalRejections, (totalRejections/totalChgTransitions)*100);
            fprintf('\n  Rejection Breakdown:\n');
            fprintf('    Short Duration:        %4d (%.1f%%) - Events < %.1f s\n', ...
                totalRejectedShortDuration, (totalRejectedShortDuration/totalChgTransitions)*100, min_duration_chg);
            fprintf('    Direction:            %4d (%.1f%%) - < 50%% positive current\n', ...
                totalRejectedDirection, (totalRejectedDirection/totalChgTransitions)*100);
            fprintf('    Too Short Window:      %4d (%.1f%%) - Event < %.1f s (stable window)\n', ...
                totalRejectedTooShortWindow, (totalRejectedTooShortWindow/totalChgTransitions)*100, 10.0);
            fprintf('    I Std:                 %4d (%.1f%%) - Current std >= %.2f A\n', ...
                totalRejectedIStd, (totalRejectedIStd/totalChgTransitions)*100, max_I_std_chg);
            fprintf('    CV:                    %4d (%.1f%%) - CV >= %.1f%%\n', ...
                totalRejectedCV, (totalRejectedCV/totalChgTransitions)*100, max_CV_chg*100);
            fprintf('    P Std:                 %4d (%.1f%%) - Power std >= %.3f kW\n', ...
                totalRejectedPStd, (totalRejectedPStd/totalChgTransitions)*100, max_P_std_chg);
            fprintf('    Too Short After Cut:    %4d (%.1f%%) - After cutting < %.1f s\n', ...
                totalRejectedTooShortAfterCut, (totalRejectedTooShortAfterCut/totalChgTransitions)*100, 5.0);
            
            % Identify top rejection reasons
            rejectionReasons = [
                totalRejectedShortDuration, ...
                totalRejectedTooShortAfterCut, ...
                totalRejectedTooShortWindow, ...
                totalRejectedIStd, ...
                totalRejectedDirection, ...
                totalRejectedCV, ...
                totalRejectedPStd
            ];
            reasonNames = {
            ...
            [sortedRejections, sortIdx] = sort(rejectionReasons, 'descend');
            fprintf('\n  Top Rejection Reasons (suggested parameters to relax):\n');
            for topIdx = 1:min(3, length(sortedRejections))
                if sortedRejections(topIdx) > 0
                    reasonName = reasonNames{sortIdx(topIdx)};
                    fprintf('    %d. %s: %d rejections (%.1f%%)\n', ...
                        topIdx, reasonName, sortedRejections(topIdx), ...
                        (sortedRejections(topIdx)/totalChgTransitions)*100);
                    
                    % Suggest parameter relaxation
                    switch reasonName
                        case 'Short Duration'
                            fprintf('       -> Consider reducing min_duration_chg from %.1f s\n', min_duration_chg);
                        case 'Too Short After Cut'
                            fprintf('       -> Consider reducing min_duration_after_cut from %.1f s\n', 5.0);
                        case 'Too Short Window'
                            fprintf('       -> Consider reducing stable_end_time from %.1f s\n', 10.0);
                        case 'I Std'
                            fprintf('       -> Consider increasing max_I_std_chg from %.2f A\n', max_I_std_chg);
                        case 'CV'
                            fprintf('       -> Consider increasing max_CV_chg from %.1f%%\n', max_CV_chg*100);
                        case 'P Std'
                            fprintf('       -> Consider increasing max_P_std_chg from %.3f kW\n', max_P_std_chg);
                        case 'Direction'
                            fprintf('       -> Consider reducing positive_ratio threshold from 50%%\n');
                    end
                end
            end
        end
