...
max_I_std = 2.0;                 % 기본값 (paramSets에서 덮어씀)
...
paramSets = struct( ...
    'name', {'set1_min30_std1_0_rng1_0', 'set2_min60_std0_5_rng0_5'}, ...
    'min_duration', {30, 60}, ...
    'max_I_std', {2.0, 0.5}, ...
    'max_I_range', {3.0, 0.5});
...
for p = 1:length(paramSets)
    ...
    outputDir = fullfile(pwd, 'Results', paramSets(p).name);
    if ~exist(outputDir, 'dir')
        mkdir(outputDir);
    end
    
    fprintf('\n=== ParamSet: %s (min_duration=%d, max_I_std=%.3f, max_I_range=%.3f) ===\n', ...
            paramSets(p).name, min_duration, max_I_std, max_I_range);
    ...
    for s = 1:length(socs)
        ...
        for p = 1:length(profs)
            ...
            [ref_events, ref_filter_stats, ref_rejected] = detect_events_in_channel(ref_I, ref_V, ref_t_seconds, current_threshold, ...
                                                   min_duration, max_I_std, max_I_range, ref_dt, debug_tag);
            ...
            for ch = 1:length(channels)
                ...
                [ch_events, ch_filter_stats, ch_rejected] = detect_events_in_channel(ch_I, ch_V, ch_t_seconds, current_threshold, ...
                                                      min_duration, max_I_std, max_I_range, ch_dt, debug_tag);
...
for d = 1:length(dcNames)
    ...
    % 사이클 목록 수집 (모든 채널/모든 SOC에서 사용된 사이클)
    allCyclesValid = {};
    allCyclesDisplay = {};
    % 표시할 SOC 열
    socNamesToShow = {'SOC90', 'SOC70', 'SOC50'};
    ...
    % 사이클별로 SOC 컬럼 테이블 생성 및 표시
    for cy = 1:length(allCyclesValid)
        cycleNameValid = allCyclesValid{cy};
        cycleDisplay = allCyclesDisplay{cy};
        fprintf('\n[Cycle: %s]\n', cycleDisplay);
        
        tableData = zeros(length(chNames), length(socNamesToShow));
        for c = 1:length(chNames)
            chName = chNames{c};
            for s = 1:length(socNamesToShow)
                socName = socNamesToShow{s};
                if isfield(filteredEventCounts.(dcName), chName) && ...
                   ...
                   isfield(filteredEventCounts.(dcName).(chName).(socName), cycleNameValid)
                    tableData(c, s) = filteredEventCounts.(dcName).(chName).(socName).(cycleNameValid);
                end
            end
        end
        
        T = array2table(tableData, 'RowNames', chNames, 'VariableNames', socNamesToShow);
        fprintf('\n');
        disp(T);
    end
end
...
function [events, filter_stats, rejected_events] = detect_events_in_channel(I, V, t_seconds, current_threshold, min_duration, max_I_std, max_I_range, dt, debug_tag)
    ...
    % 1. 노이즈 제거 (Savitzky-Golay, 3차, 윈도우 10 샘플)
    sg_order = 3;
    sg_window = 10;
    frame_len = min(sg_window, length(I));
    if mod(frame_len, 2) == 0
        frame_len = frame_len - 1;
    end
    if frame_len <= sg_order
        frame_len = sg_order + 2;
        if mod(frame_len, 2) == 0
            frame_len = frame_len + 1;
        end
    end
    if length(I) >= frame_len
        I_filt = sgolayfilt(I, sg_order, frame_len);
        V_filt = sgolayfilt(V, sg_order, frame_len);
    else
        I_filt = I;
        V_filt = V;
    end
