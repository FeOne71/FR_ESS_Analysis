                ...
                for eventType = {'Charge', 'Discharge'}
                    ...
                    for evt_idx = 1:length(events)
                        ...
                        if hasData
                            ...
                            % Add to table data
                            % Ensure consistent field structure by checking if this is the first row
                            if isempty(tableData)
                                % First row: create structure array with all fields
                                tableData = rowData;
                            else
                                % Ensure rowData has all fields from previous rows
                                existingFields = fieldnames(tableData);
                                newFields = fieldnames(rowData);
                                
                                % Add missing fields to rowData (set to NaN)
                                missingFields = setdiff(existingFields, newFields);
                                for f = 1:length(missingFields)
                                    rowData.(missingFields{f}) = NaN;
                                end
                                
                                % Add missing fields to existing tableData (set to NaN)
                                missingFields2 = setdiff(newFields, existingFields);
                                if ~isempty(missingFields2)
                                    for f2 = 1:length(missingFields2)
                                        for r = 1:length(tableData)
                                            tableData(r).(missingFields2{f2}) = NaN;
                                        end
                                    end
                                end
